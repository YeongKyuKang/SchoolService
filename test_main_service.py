...
class TestMainService(unittest.TestCase):
    ...
    def test_course_registration_redirect(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn('kangyk.com/course-service/course_registration', response.location)

    def test_festival_redirect(self):
        response = self.app.get('/festival')
        self.assertEqual(response.status_code, 302)
        self.assertIn('kangyk.com/festival-service/', response.location)

    def test_news_redirect(self):
        response = self.app.get('/news')
        self.assertEqual(response.status_code, 302)
        self.assertIn('kangyk.com/notice-service/news', response.location)

    def test_logout_redirect(self):
        response = self.app.get('/logout')
        self.assertEqual(response.status_code, 302)
        self.assertIn('kangyk.com/main-service/main', response.location)
