...
class TestFestivalService(unittest.TestCase):
    ...
    def test_search_courses(self):
        response = self.client.get('/search_courses?course_name=Introduction')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertTrue(data['success'])
    ...
    def test_apply_course(self):
        response = self.client.post('/apply_course', 
    ...
    def test_cancel_course(self):
        # 먼저 과목을 신청합니다
        self.client.post('/apply_course', 
                         data=json.dumps({'course_key': 'CS101'}),
                         content_type='application/json')

        # 이제 과목을 취소합니다
        response = self.client.post('/cancel_course', 
                                    data=json.dumps({'course_key': 'CS101'}),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
